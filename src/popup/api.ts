...
export const api = {
  ...
  tokens: (address: string) => call<{ symbol: string; balance: string; native: boolean; address: string; confidential?: boolean; privCipher?: string }[]>({ type: 'tokens', address }),
  ...
  octPrice: () => call<{ usd: number; change24h: number; mcap?: number; vol?: number }>({ type: 'octPrice' }),
  octChart: () => call<number[]>({ type: 'octChart' }),
  tokenSupply: (token: string) => call<string>({ type: 'tokenSupply', token }),
  txReceipt: (hash: string) => call<{ success: boolean | null; error?: string | null }>({ type: 'txReceipt', hash }),
  prices: () => call<Record<string, number>>({ type: 'prices' }),
  priceSeries: (token: string) => call<number[]>({ type: 'priceSeries', token }),
  swapQuote: (tokenIn: string, tokenOut: string, amountInMicro: string) =>
    call<{ found: boolean; amountOutMicro?: string; fee?: number; pool?: string; priceImpactBps?: number }>({ type: 'swapQuote', tokenIn, tokenOut, amountInMicro }),
  swap: (address: string, tokenIn: string, tokenOut: string, amountInMicro: string, minOutMicro: string, fee: number) =>
    call<{ hash: string }>({ type: 'swap', address, tokenIn, tokenOut, amountInMicro, minOutMicro, fee }),
  ...
  approvalPending: () => call<{ id: string; kind: 'connect' | 'tx' | 'sign' | 'fheprove' | 'fhedecrypt' | 'fhedeposit' | 'stealthsend' | 'stealthscan' | 'stealthviewpub'; origin: string; data: any } | null>({ __approvalPending: true }),
  approvalResolve: (id: string, approved: boolean, address?: string) => call({ __approvalResult: true, id, approved, address }),
  fheProveResolve: (id: string, approved: boolean, result?: any) => call({ __fheProveResult: true, id, approved, result }),
