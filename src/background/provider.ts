 ...
 *
 * Держим ЧУТЬ МЕНЬШЕ, чем ждёт сама страница (в entrypoints/inpage.ts стоит три
 * минуты). Иначе выходит нелепость: страница уже сдалась и сказала человеку
 * «не дождались», а место у нас всё ещё занято ожидающим запросом — и следующая
 * попытка отвергается со словами «уже есть ожидающий запрос». Две минуты, когда
 * обмен не работает без всякой причины.
 *
 * Если меняешь одно — меняй и другое. */
const ЖДЁМ_РЕШЕНИЯ = 170_000
