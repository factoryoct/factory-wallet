...
import { bytesToHex, hexToBytes, base64ToBytes } from './encoding'
...
interface Serialized { version: 1; accounts: { label: string; seedHex: string }[] }

export class Keyring {
  private accounts: Account[] = []

  private add(seed: Uint8Array, label: string): Account {
    ...
    const acct: Account = { address, label, seedHex: bytesToHex(seed) }
  ...
  createAccount(label = `Account ${this.accounts.length + 1}`): Account {
    return this.add(generateKeypair().privateKey, label)
  }

  /** Import from a 32-byte seed (hex, with or without 0x). */
  importSeed(seedHex: string, label = `Imported ${this.accounts.length + 1}`): Account {
  ...
  importMnemonic(phrase: string, label = `Imported ${this.accounts.length + 1}`): Account {
    ...
    const mac = hmac(sha512, new TextEncoder().encode('Octra seed'), mnemonicToSeedSync(m))
    return this.add(mac.slice(0, 32), label)
  }
  ...
  serialize(): string {
    const s: Serialized = { version: 1, accounts: this.accounts.map(({ label, seedHex }) => ({ label, seedHex })) }
  ...
  static deserialize(json: string): Keyring {
    ...
    for (const a of s.accounts) kr.importSeed(a.seedHex, a.label)
    return kr
  }
}
